...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        editFilePath.setHint("https://.../playlist.m3u8 or /sdcard/Movies/test.mp4");

        btnPlay.setOnClickListener(
                v -> {
                    String path = editFilePath.getText().toString().trim();
                    if (path.isEmpty()) {
                        Toast.makeText(this, "Please enter a file path or URL", Toast.LENGTH_SHORT)
                                .show();
                        return;
                    }
                    if (!isRemoteUrl(path) && !new File(path).exists()) {
    ...
    private void handleIntent(Intent intent) {
        ...
        boolean audioOnly = data.getBooleanQueryParameter("audio_only", false);
        if (path != null) {
