...
enum {
  ...
  // To programmatically set these array, use the MetaDataBase::setData API,
  // i.e. const size_t encSizes[]; meta->setData(
  ...
  // Similar to MediaFormat.KEY_IS_FORCED_SUBTITLE but pertains to av tracks as
  // well.
  ...
  // color aspects
  kKeyColorRange =
      'cRng',  // int32_t, color range, value defined by ColorAspects.Range
  kKeyColorPrimaries =
      'cPrm',  // int32_t,
               // color Primaries, value defined by ColorAspects.Primaries
  kKeyTransferFunction =
      'tFun',  // int32_t,
  ...
  kKeyTemporalLayerId =
      'iLyr',  // int32_t, temporal layer-id. 0-based (0 => base layer)
  kKeyTemporalLayerCount =
      'cLyr',  // int32_t, number of temporal layers encoded
  ...
  kKeyIsPrimaryImage =
      'prim',               // bool (int32_t), image track is the primary image
  kKeyFrameCount = 'nfrm',  // int32_t, total number of frame in video track
  kKeyExifOffset = 'exof',  // int64_t, Exif data offset
  kKeyExifSize = 'exsz',    // int64_t, Exif data size
  kKeyExifTiffOffset = 'thdr',  // int32_t, if > 0, buffer contains exif data
                                // block with tiff hdr at specified offset
  ...
  /* MediaRecorder.h, error notifications can represent track ids with 4 bits
   * only. | track id | reserved |     error or info type     | 31         28 16
   * 0
   */
  ...
  kKeySelfID =
      'sfid',  // int32_t, source ID to identify itself on RTP protocol.
  ...
  kKeyRtpDscp =
      'dscp',  // int32_t, DSCP(Differentiated services codepoint) of RFC 2474.
  kKeySocketNetwork =
      'sNet',  // int64_t, socket will be bound to network handle.
