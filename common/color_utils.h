...
struct ColorUtils {
  /*
   * Media-platform color constants. MediaCodec uses (an extended version of)
   * platform-defined constants that are derived from HAL_DATASPACE, since these
   * are directly exposed to the user. We extend the values to maintain the
   * richer set of information defined inside media containers and bitstreams
   * that are not supported by the platform. We also expect vendors to extend
   * some of these values with vendor-specific values. These are separated into
   * a vendor-extension section so they won't collide with future platform
   * values.
   */
  ...
  enum ColorStandard : uint32_t {
    ...
    /* This marks a section of color-standard values that are not supported by
       graphics HAL, but track defined color primaries-matrix coefficient
       combinations in media. These are stable for a given release. */
    kColorStandardExtendedStart = 64,

    /* This marks a section of color-standard values that are not supported by
       graphics HAL nor using media defined color primaries or matrix
       coefficients. These may differ per device. */
  ...
  enum ColorTransfer : uint32_t {
    ...
    /* This marks a section of color-transfer values that are not supported by
       graphics HAL, but track media-defined color-transfer. These are stable
       for a given release. */
    kColorTransferExtendedStart = 32,

    /* This marks a section of color-transfer values that are not supported by
       graphics HAL nor defined by media. These may differ per device. */
  ...
  static int32_t wrapColorAspectsIntoColorTransfer(
      ColorAspects::Transfer transfer);

  // unwrap methods change invalid aspects to the Other value
  static status_t unwrapColorAspectsFromColorRange(int32_t range,
  ...
  static void convertCodecColorAspectsToIsoAspects(const ColorAspects& aspects,
                                                   int32_t* primaries,
                                                   int32_t* transfer,
                                                   int32_t* coeffs,
                                                   bool* fullRange);
  // converts unsupported values to Other
  static void convertIsoColorAspectsToCodecAspects(int32_t primaries,
                                                   int32_t transfer,
                                                   int32_t coeffs,
                                                   bool fullRange,
                                                   ColorAspects& aspects);
  static void convertIsoColorAspectsToPlatformAspects(int32_t primaries,
                                                      int32_t isotransfer,
                                                      int32_t coeffs,
                                                      bool fullRange,
                                                      int32_t* range,
  ...
  static void setDefaultCodecColorAspectsIfNeeded(ColorAspects& aspects,
                                                  int32_t width,
                                                  int32_t height);

  // it returns the closest dataSpace for given color |aspects|. if |mayExpand|
  // is true, it allows returning a larger dataSpace that contains the color
  // space given by |aspects|, and is better suited to blending. This requires
  // implicit color space conversion on part of the device.
  // TODO(youfa) android_dataspace
  // static android_dataspace getDataSpaceForColorAspects(ColorAspects& aspects,
  //                                                     bool mayExpand);
  ...
  // compares |aspect| to |orig|. Returns |true| if any aspects have changed,
  // except if they changed to Unspecified value. It also sets the changed
  // values to Unspecified in |aspect|.
  static bool checkIfAspectsChangedAndUnspecifyThem(
      ColorAspects& aspects,
      const ColorAspects& orig,
      bool usePlatformAspects = false);
  ...
  static void getColorAspectsFromFormat(const std::shared_ptr<Message>& format,
                                        ColorAspects& aspects);

  // writes |aspects| into format. iff |force| is false, Unspecified values are
  // not written.
  static void setColorAspectsIntoFormat(const ColorAspects& aspects,
                                        std::shared_ptr<Message>& format,
                                        bool force = false);

  // finds HDR metadata in format as HDRStaticInfo, defaulting them to 0.
  // Return |true| if could find HDR metadata in format. Otherwise, return
  // |false|.
  static bool getHDRStaticInfoFromFormat(const std::shared_ptr<Message>& format,
                                         HDRStaticInfo* info);

  // writes |info| into format.
  static void setHDRStaticInfoIntoFormat(const HDRStaticInfo& info,
                                         std::shared_ptr<Message>& format);
  // writes |info| into format.
  // TODO(youfa) AMediaFormat
  // static void setHDRStaticInfoIntoAMediaFormat(const HDRStaticInfo& info,
  //                                             AMediaFormat* format);
...
inline static const char* asString(avp::ColorUtils::ColorStandard i,
  ...
  switch (i) {
    ...
    case ColorUtils::kColorStandardBT601_625:
      return "BT601_625";
    case ColorUtils::kColorStandardBT601_625_Unadjusted:
      return "BT601_625_Unadjusted";
    case ColorUtils::kColorStandardBT601_525:
      return "BT601_525";
    case ColorUtils::kColorStandardBT601_525_Unadjusted:
      return "BT601_525_Unadjusted";
    case ColorUtils::kColorStandardBT2020:
      return "BT2020";
    case ColorUtils::kColorStandardBT2020Constant:
      return "BT2020Constant";
    case ColorUtils::kColorStandardBT470M:
      return "BT470M";
    case ColorUtils::kColorStandardFilm:
      return "Film";
    case ColorUtils::kColorStandardDCI_P3:
      return "DCI_P3";
    default:
      return def;
  }
...
inline static const char* asString(avp::ColorUtils::ColorTransfer i,
  ...
  switch (i) {
    case ColorUtils::kColorTransferUnspecified:
      return "Unspecified";
    case ColorUtils::kColorTransferLinear:
      return "Linear";
    case ColorUtils::kColorTransferSRGB:
      return "SRGB";
    case ColorUtils::kColorTransferSMPTE_170M:
      return "SMPTE_170M";
    case ColorUtils::kColorTransferGamma22:
      return "Gamma22";
    case ColorUtils::kColorTransferGamma28:
      return "Gamma28";
    case ColorUtils::kColorTransferST2084:
      return "ST2084";
    case ColorUtils::kColorTransferHLG:
      return "HLG";
    case ColorUtils::kColorTransferGamma26:
      return "Gamma26";
    default:
      return def;
  }
...
inline static const char* asString(avp::ColorUtils::ColorRange i,
  ...
  switch (i) {
    ...
    case ColorUtils::kColorRangeFull:
      return "Full";
    case ColorUtils::kColorRangeLimited:
      return "Limited";
    default:
      return def;
  }
